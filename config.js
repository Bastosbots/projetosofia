...
// Sobe patente por nivel, mude pro que quiser dentro das aspas
		...
		case 'grupo':
            ...
            if (onar[0] == 'fechar') {
				kill.setGroupToAdminsOnly(groupId, true).then(() => kill.sendText(from, '⚠️Grupo fechado para evitar incidentes desagradáveis durante a madrugada \n\n ⚠️O grupo será reaberto pela manhã assim que o primeiro adm acordar \n\n ⚠️ *Pedimos a compreensão de todos*'))
			} else if (onar[0] == 'abrir') {
				kill.setGroupToAdminsOnly(groupId, false).then(() => kill.sendText(from, '*☀️O Grupo foi reaberto :), vamos interagir!☀️*'))
			} else {
       ...
       case 'clima':
        ...
        case 'promote':
			if (isGroupMsg && isGroupAdmins || isGroupMsg && isOwner) {
				...
				if (quotedMsg) {
					...
					await kill.sendTextWithMentions(from, `O usuário @${proquo} se tornou um administrador.`)
					...
					await kill.sendTextWithMentions(from, `O Usuário @${mentionedJidList[0]} se tornou um administrador.`)
				}
			} else if (isGroupMsg) {
				await kill.reply(from, 'Desculpe, somente os administradores podem usar esse comando...', id)
			} else {
        ...
        case 'demote':
			if (isGroupMsg && isGroupAdmins || isGroupMsg && isOwner) {
				...
				if (quotedMsg) {
					...
					await kill.sendTextWithMentions(from, `Demitindo administrador @${demquo}.`)
					...
					await kill.sendTextWithMentions(from, `Demitindo administrador @${mentionedJidList[0]}.`)
        ...
        case 'exclusive':
            if (!isGroupMsg) return kill.reply(from, mess.error.Gp, id)
			if (!isGroupAdmins) return kill.reply(from, 'Esse comando é apenas para meu criador', id)
            if (args.length !== 1) return kill.reply(from, 'Defina entre on e off!', id)
			if (args[0] == 'on') {
                exsv.push(chatId)
        ...
        case 'menu':
            ...
            kill.reply(from, `_Estes comandos foram feitos para ajudar na organização e na interação do grupo, por favor use com cautela e não faça bagunça para não ser banido(a)! :)_`, id)
            break
		...
		case 'ranking':
            ...
            try {
                for (let i = 0; i < 10; i++) {
					...
					if (cklvl[i].level >= 1) {
						role = 'Bronze II'
					} else if (cklvl[i].level >= 2) {
						role = 'Bronze III'
					} else if (cklvl[i].level >= 3) {
						role = 'Bronze IV'
					} else if (cklvl[i].level >= 4) {
						role = 'Bronze V'
					} else if (cklvl[i].level >= 5) {
						role = 'Prata I'
					} else if (cklvl[i].level >= 6) {
						role = 'Prata II'
					} else if (cklvl[i].level >= 7) {
						role = 'Prata III'
					} else if (cklvl[i].level >= 8) {
						role = 'Prata IV]'
					} else if (cklvl[i].level >= 9) {
						role = 'Prata V'
					} else if (cklvl[i].level >= 10) {
						role = 'Ouro I'
					} else if (cklvl[i].level >= 11) {
						role = 'Ouro II'
					} else if (cklvl[i].level >= 12) {
						role = 'Ouro III'
					} else if (cklvl[i].level >= 13) {
						role = 'Ouro IV'
					} else if (cklvl[i].level >= 14) {
						role = 'Ouro V'
					} else if (cklvl[i].level >= 15) {
						role = 'Diamante I'
					} else if (cklvl[i].level >= 16) {
						role = 'Diamante II'
					} else if (cklvl[i].level >= 17) {
						role = 'Diamante III'
					} else if (cklvl[i].level >= 18) {
						role = 'Diamante IV'
					} else if (cklvl[i].level >= 19) {
						role = 'Diamante V]'
                ...
                board += `${i + 1}. @${nivel[i].id.replace('@c.us/g', '')}\n➸ *XP*: ${nivel[i].xp}\n➸ *Level*: ${nivel[i].level}\n➸ *Patente*: ${role}\n\n`
                }
